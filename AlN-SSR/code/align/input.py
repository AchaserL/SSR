...
def read_dbp15k_input(folder):
    triples_set1 = read_triples(folder + 'triples_1')  # 源KG三元组集
    triples_set2 = read_triples(folder + 'triples_2')  # 目标KG三元组集
    kg1 = KG(triples_set1)  # 源KG对象
    kg2 = KG(triples_set2)  # 目标KG对象
    total_ent_num = len(kg1.ents | kg2.ents)    # 两个图谱的实体总数
    total_rel_num = len(kg1.props | kg2.props)  # 两个图谱的关系总数
    total_triples_num = len(kg1.triple_list) + len(kg2.triple_list)  # 三元组总数
    print('total ents:', total_ent_num)
    ...
    if os.path.exists(folder + 'ref_pairs'):  # 测试集
        ref_ent1, ref_ent2 = read_references(folder + 'ref_pairs')
    else:
        ref_ent1, ref_ent2 = read_references(folder + 'ref_ent_ids')
    assert len(ref_ent1) == len(ref_ent2)
    print("To aligned entities:", len(ref_ent1))
    if os.path.exists(folder + 'sup_pairs'):  # 训练集
