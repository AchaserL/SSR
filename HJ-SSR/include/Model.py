...
def build(dimension, act_func, gamma, k, lang, e, ILL, KG):
    ...
    input_layer = get_input_layer(e, dimension, lang) # 输入层
    M = get_sparse_tensor(e, KG) # 邻接矩阵
    head, tail, head_r, tail_r = rfunc(KG, e)
    
    print('calculate preliminary entity representations') # 计算初始实体表示
...
def training(output_prel_e, output_joint_e, output_r, loss_1, loss_2, learning_rate, epochs, ILL, e, k, s, test, test_r, head, tail, adj):
    ...
    if os.path.exists("./coinc.npy"):
        coinc = np.load("./coinc.npy", allow_pickle=True)
    else:
        coinc = detect_coinc(test_r, head, tail, ILL)
        np.save("./coinc.npy", coinc)
    ...
    for i in range(epochs):
        if i < s:  # preliminary training
            ...
            if i % 10 == 0:
                th, outvec_e, outvec_r = sess.run([loss_1, output_prel_e, output_r], feed_dict=feeddict)
                J.append(th)
                get_hits(outvec_e, test)
            if i == epochs - 1:
                _, outvec_e, _ = sess.run([loss_1, output_prel_e, output_r], feed_dict=feeddict)
                for weight in range(0, 55, 5):
                    weight /= 100
                    gabs(outvec_e, test, Config.can_len, adj, weight)
            # if i == epochs - 1:
            #     th, outvec_e, outvec_r = sess.run([loss_1, output_prel_e, output_r], feed_dict=feeddict)
            #     for weight in range(0, 55, 5):
            #         weight /= 100
            ...
            if i % 10 == 0:
                th, outvec_e, outvec_r = sess.run([loss_2, output_joint_e, output_r], feed_dict=feeddict)
                J.append(th)
                get_hits(outvec_e, test)
            if i == epochs - 1:
                th, outvec_e, outvec_r = sess.run([loss_2, output_joint_e, output_r], feed_dict=feeddict)
